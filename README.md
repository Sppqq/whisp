...
- экран «Сегодня» с ближайшими занятиями, заданиями и интервальным повторением;
...
Whisp проверяет GitHub Releases при запуске. На странице **«Обновления»** в настройках можно отключить автоматическую проверку и выбрать канал **стабильных релизов** или **Beta / prerelease**. Там же отображается прогресс загрузки, можно прочитать changelog и скачать новый DMG. После загрузки приложение само откроет образ; останется перетащить Whisp в Applications с заменой. Записи и настройки при обновлении сохраняются.

Проверка начнёт работать после перевода репозитория в публичный доступ: приватный GitHub Releases API требует авторизацию, которой приложение намеренно не запрашивает.

### Провайдеры AI

В настройках можно выбрать Google Gemini, OpenAI, Anthropic, xAI, OpenRouter или свой OpenAI-совместимый endpoint. Для OpenAI-совместимых сервисов Whisp использует `/chat/completions` и `/audio/transcriptions`; Anthropic используется для генерации конспектов, а расшифровка аудио для него недоступна через официальный API.
