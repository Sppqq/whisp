...
> [!WARNING]
> Текущая ad-hoc сборка сохраняет введённые ключи и пароли в локальном `UserDefaults`, чтобы пересборки не вызывали повторные запросы Keychain. Эти значения не попадают в Git, но не зашифрованы на диске. Используйте отдельные ключи с минимальными правами и не передавайте профиль пользователя другим людям.
